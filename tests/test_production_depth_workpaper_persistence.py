...
def test_workpaper_persistence_api_round_trip(monkeypatch, tmp_path: Path) -> None:
    ...
    fetched_narrative = client.get(f"/api/v1/civicbudget/narrative/{narrative_id}")
    ...
    fetched_packet = client.get(f"/api/v1/civicbudget/hearing-packet/{packet_id}")
...
def test_get_narrative_without_persistence_returns_actionable_503(monkeypatch) -> None:
    ...
    assert detail["message"] == "CivicBudget workpaper persistence is not configured."
    assert "Set CIVICBUDGET_WORKPAPER_DB_URL" in detail["fix"]
...
def test_get_hearing_packet_missing_id_returns_actionable_404(monkeypatch, tmp_path: Path) -> None:
    ...
    response = client.get("/api/v1/civicbudget/hearing-packet/missing")
